...
public class Loan {
    ...
    public Loan(AbsLoan l) {
        id = l.getId();
        category = l.getAbsCategory();
        ownerName = l.getAbsOwner();
...
//        int date = timeline.getCurrentDate();
//        return (getNextPayment(date) == date+monthsPerPayment);
//    }
    ...
    public void payout()
    {
            for (Customer cus : initializer.customerList)
            {
                if (cus.getName().equals(this.ownerName))
                {
                    ...
                    {
                        involvedCustomers.forEach((key, val) -> getItDone(key, val, true));
                    ...
                    {
                        involvedCustomers.forEach((key, val) -> getItDone(key, val, false));
                    }
                }
            }
    }

    private void getItDone(Customer cus, int val, boolean paid){
